...
function Vert_Coordinate(; nλ::Int64, nθ::Int64, nd::Int64,
    ...
    ak, bk = Compute_Vert_Coord(nd, vert_coord_option, p_ref, zero_top, scale_heights,
                                surf_res, p_press, p_sigma, exponent)
    Δak, Δbk = ak[2:(nd + 1)] - ak[1:nd], bk[2:(nd + 1)] - bk[1:nd]

    flux = zeros(Float64, nλ, nθ, nd + 1)
    vert_integral = zeros(Float64, nλ, nθ, 1)
    return Vert_Coordinate(nλ, nθ, nd, vert_coord_option, vert_difference_option,
                           vert_advect_scheme, p_ref, zero_top, ak, bk, Δak, Δbk,
                           flux, vert_integral)
end

function Compute_Vert_Coord(nd::Int64, vert_coord_option::String,
                            p_ref::Float64=101325.0, zero_top::Bool=true,
                            scale_heights::Float64=4.0, surf_res::Float64=1.0,
                            p_press::Float64=0.1, p_sigma::Float64=0.3,
                            exponent::Float64=2.5)
...
function Compute_Uneven_Sigma(nd::Int64, scale_heights::Float64, surf_res::Float64,
                              exponent::Float64, zero_top::Bool)
    """
    ζ = 1 - (k-1]/nd
...
function Vert_Advection!(vert_coord::Vert_Coordinate, r::Array{Float64,3},
                         dz::Array{Float64,3}, w::Array{Float64,3}, Δt::Int64,
                         vert_advect_scheme::String, rdt::Array{Float64,3})
    """
    ...
    return rdt[:, :, 1:nd] .= (flux[:, :, 1:nd] - flux[:, :, 2:(nd + 1)] +
                               r[:, :, 1:nd] .* (w[:, :, 2:(nd + 1)] - w[:, :, 1:nd])) ./
                              dz[:, :, 1:nd]
end

function Mass_Weighted_Global_Integral(vert_coord::Vert_Coordinate,
