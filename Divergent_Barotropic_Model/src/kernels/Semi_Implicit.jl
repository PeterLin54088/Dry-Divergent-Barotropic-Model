export Semi_Implicit_Solver, Build_Implicit_Matrices, Build_Wave_Matrices,
       Update_Init_Step!, Linear_Press_Gradient_δps, Linear_Geopot_δt!,
...
function Semi_Implicit_Solver(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
                              integrator::Filtered_Leapfrog, ps_ref::Float64,
                              t_ref::Array{Float64,1},
                              wave_numbers::Array{Int64,2})
    nd, ak, bk = vert_coord.nd, vert_coord.ak, vert_coord.bk
    ps_ref_3d = reshape([ps_ref], (1, 1, 1))
    Δp_ref, p_half_ref, lnp_half_ref, p_full_ref, lnp_full_ref = zeros(Float64, (1, 1, nd)),
                                                                 zeros(Float64,
                                                                       (1, 1, nd + 1)),
                                                                 zeros(Float64,
                                                                       (1, 1, nd + 1)),
                                                                 zeros(Float64, (1, 1, nd)),
                                                                 zeros(Float64, (1, 1, nd))

    #using ps_ref compute lnp_half_ref lnp_full_ref
    Pressure_Variables!(vert_coord, ps_ref_3d, p_half_ref, Δp_ref, lnp_half_ref, p_full_ref,
                        lnp_full_ref)

    Δp_ref, p_half_ref, lnp_half_ref, p_full_ref, lnp_full_ref = Δp_ref[1, 1, :],
                                                                 p_half_ref[1, 1, :],
                                                                 lnp_half_ref[1, 1, :],
                                                                 p_full_ref[1, 1, :],
                                                                 lnp_full_ref[1, 1, :]
    ...
    h, div_mat = Build_Implicit_Matrices(vert_coord, atmo_data,
                                         ps_ref, Δp_ref,
                                         lnp_half_ref, lnp_full_ref, δlnp_half_ref,
                                         δlnp_full_ref, t_ref)
    ...
    spe_δgeopot_half_temp = Array{ComplexF64,3}(undef, num_fourier + 1, num_spherical + 1,
                                                nd + 1)
    ...
    spe_Mdt_half_temp = Array{ComplexF64,3}(undef, num_fourier + 1, num_spherical + 1,
                                            nd + 1)

    return Semi_Implicit_Solver(integrator, nd, ps_ref, Δp_ref, lnp_half_ref, lnp_full_ref,
                                t_ref,
                                h, div_mat, num_wavenumbers, wave_numbers, wave_matrix,
                                spe_δdiv_temp, spe_δps_temp, spe_δt_temp,
                                spe_δgeopot_half_temp, spe_δgeopot_temp, spe_M_half_temp,
                                spe_Mdt_half_temp)
end
...
function Build_Implicit_Matrices(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
    ...
    spe_zero_half, spe_zero_full = zeros(ComplexF64, 1, 1, nd + 1),
    ...
    spe_M_half_temp, spe_Mdt_half_temp = zeros(ComplexF64, 1, 1, nd + 1),
                                         zeros(ComplexF64, 1, 1, nd + 1)
    for k in 1:nd
        spe_input .= 0.0
        spe_input[1, 1, k] = 1.0

        Linear_Ps_T_δdiv!(vert_coord, atmo_data, spe_input, ps_ref, Δp_ref, lnp_half_ref,
                          lnp_full_ref, t_ref,
        ...
        Linear_Geopot_δt!(vert_coord, atmo_data, lnp_half_ref, lnp_full_ref, spe_input,
                          t_ref, spe_δgeopot_half, spe_δgeopot)
        gamma[:, k] = spe_δgeopot[1, 1, :]
    end

    #used only once
    h1 = Linear_Press_Gradient_δps(vert_coord, atmo_data, ps_ref, Δp_ref, lnp_half_ref,
                                   lnp_full_ref, t_ref)
    h2 = Linear_Geopot_δps(vert_coord, atmo_data, δlnp_half_ref, δlnp_full_ref,
                           lnp_half_ref, lnp_full_ref, t_ref)
...
function Build_Wave_Matrices(num_total_wavenumbers::Int64, eigen::Array{Float64,2},
                             div_mat::Array{Float64,2}, ξ::Float64)
    nd = size(div_mat)[1]
    wave_matrix = zeros(Float64, nd, nd, num_total_wavenumbers + 1)

    for i in 0:num_total_wavenumbers
...
function Update_Init_Step!(semi_implicit::Semi_Implicit_Solver)
    ...
    ξ = Get_ξ(integrator)
    return semi_implicit.wave_matrix .= Build_Wave_Matrices(semi_implicit.num_wavenumbers,
                                                            integrator.laplacian_eigen,
                                                            semi_implicit.div_mat, ξ)
end
...
function Linear_Press_Gradient_δps(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
    ...
    if (vert_difference_option == "simmons_and_burridge")
        for k in 1:nd
            Δlnp_p = lnp_half_ref[k + 1] - lnp_full_ref[k]
            Δlnp_m = lnp_full_ref[k] - lnp_half_ref[k]
            R_T_δlnp[k] = rdgas * t_ref[k] * (bk[k + 1] * Δlnp_p + bk[k] * Δlnp_m) /
                          Δp_ref[k]
        end
    end
...
function Linear_Geopot_δt!(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
    ...
    for k in nd:-1:2
        ...
        spe_δgeopot_half[:, :, k] .= spe_δgeopot_half[:, :, k + 1] +
                                     rdgas * (spe_δt[:, :, k] *
                                              (lnp_half_ref[k + 1] - lnp_half_ref[k]))
    end

    for k in 1:nd
        #todo optimize
        #Φ_{k} = Φ_{k+1/2} + RT_k(ln p_{k+1/2} - ln p_{k})
        spe_δgeopot[:, :, k] .= spe_δgeopot_half[:, :, k + 1] +
                                rdgas *
                                (spe_δt[:, :, k] * (lnp_half_ref[k + 1] - lnp_full_ref[k]))
    end
end
...
function Linear_Geopot_δps(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
    ...
    for k in nd:-1:2
        δgeopot_half_ref[k] = δgeopot_half_ref[k + 1] +
                              rdgas * (t_ref[k] * (δlnp_half_ref[k + 1] - δlnp_half_ref[k]))
    end

    for k in 1:nd
        δgeopot_ref[k] = δgeopot_half_ref[k + 1] +
...
function Linear_Ps_T_δdiv!(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
    ...
    if (vert_difference_option == "simmons_and_burridge")
        for k in 1:nd
            ...
            spe_δt[:, :, k] .= -kappa * t_ref[k] * (dmean_tot * Δlnp + dmean * Δlnp_p) /
                               Δp_ref[k]
            # dmean_tot = ∑_r=1^k Dr
            dmean_tot .+= dmean
    ...
    for k in 1:nd
        spe_δt[:, :, k] .-= 0.5 * (spe_Mdt_half[:, :, k + 1] + spe_Mdt_half[:, :, k]) /
                            Δp_ref[k]
    end
end
...
function Adjust_δlnps_δt_δdiv!(semi_implicit::Semi_Implicit_Solver,
                               ...
                               spe_δdiv::Array{ComplexF64,3},
                               spe_δlnps::Array{ComplexF64,3}, spe_δt::Array{ComplexF64,3})
    ...
    spe_M_half_temp, spe_Mdt_half_temp = semi_implicit.spe_M_half_temp,
                                         semi_implicit.spe_Mdt_half_temp

    # compute linearized δt, with constant surface pressure 
    Linear_Ps_T_δdiv!(vert_coord, atmo_data, spe_δdiv_temp, ps_ref, Δp_ref, lnp_half_ref,
                      lnp_full_ref, t_ref,
    ...
    Linear_Geopot_δt!(vert_coord, atmo_data, lnp_half_ref, lnp_full_ref, spe_δt_temp, t_ref,
                      spe_δgeopot_half_temp, spe_δgeopot_temp)

    nd = vert_coord.nd

    for k in 1:nd
        spe_δdiv[:, :, k] .-= laplacian_eigen .* (spe_δgeopot_temp[:, :, k] .+
                                                  h[k] * ps_ref * spe_δlnps_temp[:, :, 1])
    end
end
...
function Implicit_Correction!(semi_implicit::Semi_Implicit_Solver,
                              vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
                              spe_div_c::Array{ComplexF64,3},
                              spe_div_p::Array{ComplexF64,3},
                              spe_lnps_c::Array{ComplexF64,3},
                              spe_lnps_p::Array{ComplexF64,3},
                              spe_t_c::Array{ComplexF64,3}, spe_t_p::Array{ComplexF64,3},
                              spe_δdiv::Array{ComplexF64,3}, spe_δlnps::Array{ComplexF64,3},
    ...
    for m in 0:(nf - 1)
        for n in m:(ns - 1)
            ...
            if (L <= num_wavenumbers)
                ...
                spe_δdiv[m + 1, n + 1, :] .= wave_matrix[:, :, L + 1] *
                                             spe_δdiv[m + 1, n + 1, :]
            end
        end
    end
    ...
    return spe_δlnps .+= ξ / ps_ref * spe_δps_temp
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    vert_coord = Vert_Coordinate(nλ, nθ, nd, "even_sigma", "simmons_and_burridge",
                                 "second_centered_wts")
