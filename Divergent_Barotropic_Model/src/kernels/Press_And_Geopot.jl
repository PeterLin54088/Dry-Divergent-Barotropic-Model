export Compute_Pressures_And_Heights!, Half_Level_Pressures!, Pressure_Variables!,
...
function Compute_Pressures_And_Heights!(atmo_data::Atmo_Data, vert_coord::Vert_Coordinate,
    ...
    Compute_Geopotential!(vert_coord, atmo_data, grid_lnp_half, grid_lnp_full, grid_t,
                          grid_geopots, grid_z_full, grid_z_half)

    grid_z_full ./= grav
    return grid_z_half ./= grav
end

function Half_Level_Pressures!(vert_coord::Vert_Coordinate, grid_ps::Array{Float64,3},
                               grid_p_half::Array{Float64,3})
    nd = vert_coord.nd
    bk = vert_coord.bk
    ak = vert_coord.ak
...
function Pressure_Variables!(vert_coord::Vert_Coordinate, grid_ps::Array{Float64,3},
                             grid_p_half::Array{Float64,3}, grid_Δp::Array{Float64,3},
                             grid_lnp_half::Array{Float64,3}, grid_p_full::Array{Float64,3},
                             grid_lnp_full::Array{Float64,3})
    ...
    if (vert_coord.vert_difference_option == "simmons_and_burridge")
        ...
        grid_lnp_full[:, :, k_top:nd] .= grid_lnp_half[:, :, (k_top + 1):(nd + 1)] .+
                                         grid_p_half[:, :, k_top:nd] .*
                                         (grid_lnp_half[:, :, (k_top + 1):(nd + 1)] -
                                          grid_lnp_half[:, :, k_top:nd]) ./
        ...
        error("vert_difference_option ", vert_coord.vert_difference_option,
              " is not a valid value for option")
    end
    return grid_p_full .= exp.(grid_lnp_full)
end
...
function Compute_Geopotential!(vert_coord::Vert_Coordinate, atmo_data::Atmo_Data,
    ...
    for k in nd:-1:k_top
        ...
        grid_geopot_half[:, :, k] .= grid_geopot_half[:, :, k + 1] .+
                                     rdgas * virtual_t[:, :, k] .*
    ...
    for k in 1:nd
        ...
        grid_geopot_full[:, :, k] .= grid_geopot_half[:, :, k + 1] .+
                                     rdgas * virtual_t[:, :, k] .*
