...
function velocity_profile(IC::String="rigid_rotation")
    ...
    if IC == "rigid_rotation"
        ...
        for k in 1:nd
            ...
            grid_div0 .= 0.0
            for i in 1:nλ
                grid_vor0[i, :, k] = 2 * U / radius *
                                     (cos(β) * sinθ - sin(β) * cos(λc[i]) * cosθ)
            end
        end
    elseif IC == "random_1"
    ...
    elseif IC == "random_2"
        ...
        for k in 1:nd
            for i in 1:nλ
                for j in 1:nθ
                    grid_u0[i, j, k] = 25 * cosθ[j] - 30 * cosθ[j]^3 +
                                       300 * sinθ[j]^2 * cosθ[j]^6   # cosθ[j]^2
                    grid_v0[i, j, k] = 0.0
                    grid_div0[i, j, k] = 0.0
                    grid_vor0[i, j, k] = -(-50 * sinθ[j] + 120 * cosθ[j]^2 * sinθ[j] +
                                           300 *
                                           (2cosθ[j]^7 * sinθ[j] - 7cosθ[j]^5 * sinθ[j]^3)) /
                                         radius # 3*sinθ[j]*cosθ[j]/radius
                end
            end
        end
    elseif IC == "mix"
        ...
        for k in 1:nd
            for i in 1:nλ
                for j in 1:nθ
                    grid_u0[i, j, 1] = U *
                                       (cos(β) * cosθ[j] + sin(β) * cos(λc[i]) * sinθ[j])
                    grid_v0[i, j, 1] = -U * sin(β) * sinλc[i]
                    grid_u0[i, j, 2] = cosθ[j]^2
                    grid_v0[i, j, 2] = 0.0
                    grid_u0[i, j, 3] = 25 * cosθ[j] - 30 * cosθ[j]^3 +
                                       300 * sinθ[j]^2 * cosθ[j]^6   # cosθ[j]^2
                    grid_v0[i, j, 3] = 0.0
                    grid_div0[i, j, 1] = 0.0
                    grid_vor0[i, j, 1] = 2 * U / radius *
                                         (cos(β) * sinθ[j] - sin(β) * cos(λc[i]) * cosθ[j])
                    grid_div0[i, j, 2] = 0.0
                    grid_vor0[i, j, 2] = 3 * sinθ[j] * cosθ[j] / radius
                    grid_div0[i, j, 3] = 0.0
                    grid_vor0[i, j, 3] = -(-50 * sinθ[j] + 120 * cosθ[j]^2 * sinθ[j] +
                                           300 *
                                           (2cosθ[j]^7 * sinθ[j] - 7cosθ[j]^5 * sinθ[j]^3)) /
                                         radius # 3*sinθ[j]*cosθ[j]/radius
                end
...
function test_derivative(IC::String="rigid_rotation")
    ...
    nλ, nθ, nd, num_fourier, num_spherical = mesh.nλ, mesh.nθ, mesh.nd, mesh.num_fourier,
                                             mesh.num_spherical
    ...
    @info "norm(div), norm(vor)", norm(div[:, :, 1]), norm(div[:, :, 2]),
          norm(div[:, :, 3]), norm(vor)
...
function test_advection(IC::String="rigid_rotation")
    ...
    for k in 1:nd
        for i in 1:nλ
            for j in 1:nθ
                grid_hs0[i, j, k] = 25 * cosθ[j] - 30 * cosθ[j]^3 +
                                    300 * sinθ[j]^2 * cosθ[j]^6 * sin(λc[i])
                grid_∇hs0[i, j, 1, k] = (300 * sinθ[j]^2 * cosθ[j]^5 * cos(λc[i])) / radius
                grid_∇hs0[i, j, 2, k] = (-25 * sinθ[j] + 90 * cosθ[j]^2 * sinθ[j] +
                                         600 * sinθ[j] * cosθ[j]^7 * sin(λc[i]) -
                                         1800 * sinθ[j]^3 * cosθ[j]^5 * sin(λc[i])) / radius
            end
        end
    end
    ...
    Vor_Div_From_Grid_UV!(mesh, grid_u0 .* grid_hs0, grid_v0 .* grid_hs0, spe_vor1,
                          spe_div1)
    ...
    @show "divergence approach error: ",
          norm(grid_δhs_ref + grid_div1 + grid_hs0 .* grid_div0)
end
